...
public interface HibernateDao {
    ...
    /**
     * 서버 커서를 사용하는 {@link ScrollableResults} 를 반환합니다.
     * 대용량 데이터에 대해 scoll을 통해 작업을 수행할 때 필요합니다.
     *
     * @param clazz entity type
     * @return {@link ScrollableResults} instance
     */
    ScrollableResults scroll(Class<?> clazz);

    /**
     * DetachedCriteria로 조회된 정보를 서버 커서를 사용하는 {@link ScrollableResults} 를 반환합니다.
     * 대용량 데이터에 대해 scoll을 통해 작업을 수행할 때 필요합니다.
     *
     * @return {@link ScrollableResults} instance
     */
     ...
     * 대용량 데이터에 대해 scoll을 통해 작업을 수행할 때 필요합니다.
     *
     * @param scrollMode scroll mode ({@link ScrollMode}
     * @return {@link ScrollableResults} instance
     */
    ScrollableResults scroll(DetachedCriteria dc, ScrollMode scrollMode);

    /**
     * Criteria로 조회된 정보를 서버 커서를 사용하는 {@link ScrollableResults} 를 반환합니다.
     * 대용량 데이터에 대해 scoll을 통해 작업을 수행할 때 필요합니다.
     *
     * @return {@link ScrollableResults} instance
     */
    ...
    /**
     * {@link Criteria} 로 조회된 정보를 서버 커서를 사용하는 {@link ScrollableResults} 를 반환합니다.
     * 대용량 데이터에 대해 scoll을 통해 작업을 수행할 때 필요합니다.
     *
     * @param scrollMode scroll mode ({@link ScrollMode}
     * @return {@link ScrollableResults} instance
    ...
    /**
     * {@link Query}로 조회된 정보를 서버 커서를 사용하는 {@link ScrollableResults} 를 반환합니다.
     * 대용량 데이터에 대해 scoll을 통해 작업을 수행할 때 필요합니다.
     *
     * @return {@link ScrollableResults} instance
     */
    ...
    /**
     * 해당 수형의 엔티티 중 질의 {@link Criteria}로 필터링된 엔티티들을 조회합니다.
     *
     * @param clazz    entity type
    ...
    /**
     * 해당 수형의 엔티티 중 질의 {@link Criteria}로 필터링된 엔티티 중 범위에 해당하는 엔티티만 조회합니다.
     *
     * @param clazz       entity type
     * @param criteria    질의 조건
     * @param firstResult first result index (start from 0)
    ...
    /**
     * 해당 수형의 엔티티 중 질의 {@link Query}로 필터링된 엔티티들을 조회합니다.
     *
     * @param clazz      entity type
     * @param query      질의
    ...
    /**
     * 해당 수형의 엔티티 중 질의 {@link Query}로 필터링된 엔티티 중 범위에 해당하는 엔티티만 조회합니다.
     *
     * @param clazz       entity type
     * @param query       질의
     * @param firstResult first result index (start from 0)
